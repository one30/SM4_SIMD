...
uint64_t buf[36][32];//enc round
uint64_t temp[36][32];//temp data
/*
...
void SM4_key_schedule(uint8_t key[16], uint32_t rkey[32])
{
    ...
    //     for(int j = 0; j<32; j++)
    //     {
    //         printf("BS_RK[%d][%d]:%016llx ",i,j,BS_RK[i][j]);
    //         if((j+1)%4==0) printf("\n");
    //     }
    // }
...
void BS_iteration()
{
    ...
    while(i < 32)//32轮迭代计算
    {
        ...
        {
            buf[4+i][j]= buf[i+1][j] ^ buf[i+2][j] ^ buf[i+3][j] ^ BS_RK[i][j];
            // printf("buf[%d][%d]=%016llx",i+4,j,buf[4+i][j]);
            // if((j+1)%4==0) printf("\n");
        }
        //printf("\ttest sbox:\n");
        S_box(i);//bingo 合成置换T的非线性变换
        //printf("\tafter shift\n");
        for(int j = 0; j < 32; j++)//bingo 4道32bit数据操作:合成置换T的线性变换L
        {
            // printf("buf[%d][%d]=%016llx ",i+4,j,buf[4+i][j]);
...
void S_box(int round)
{
    ...
    for(int i = 0; i<4; i++)
    {
        // sm4.b0 = buf[round+4][i*8];
        // sm4.b1 = buf[round+4][i*8+1];
        // sm4.b2 = buf[round+4][i*8+2];
        // sm4.b3 = buf[round+4][i*8+3];
        // sm4.b4 = buf[round+4][i*8+4];
        // sm4.b5 = buf[round+4][i*8+5];
        // sm4.b6 = buf[round+4][i*8+6];
        // sm4.b7 = buf[round+4][i*8+7];
        ...
        // for(int j = 0; j<8; j++)
        // {
        //     printf("sbox：%016llx",buf[round+4][i*8+j]);
        //     if((j+1)%4==0) printf("\n");
        // }
    }
    //for(int )
